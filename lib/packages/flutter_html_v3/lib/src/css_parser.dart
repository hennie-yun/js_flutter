    ...
    if (value.isNotEmpty) {
      switch (property) {
        ...
        case 'border-bottom':
          ...
          break;
        case 'border-radius':
          // ToDO. 일부 Radius 넣을 수 있도록 변경하기
          style.borderRadius =
              ExpressionMapping.expressionToBorderRadius(value);
          break;
        case 'color':
          style.color =
        ...
        case 'justify-content':
          final justifyContent =  (value.first as css.LiteralTerm).text;
          print('css_parser : $justifyContent');
          if (justifyContent == 'end') {
            style.justifyContent = Alignment.centerRight;
          } else if (justifyContent == 'center') {
            style.justifyContent == Alignment.center;
          }
          print('css_parser222');
          print(style.justifyContent );

          // style.justifyContent =
          //     ExpressionMapping.expressionToJustifyContents(value.first);
...
class ExpressionMapping {
  ...
  // TODO  수정 중
  static BorderRadius? expressionToBorderRadius(
      List<css.Expression> radiusList) {
    //왼쪽 상단, 오른쪽 상단, 왼쪽 하단, 오른쪽 하단 값을 순서대로 저장
    List<double> radiusValueList = [0.0, 0.0, 0.0, 0.0];

    radiusList.asMap().forEach((index, element) {
      if (element is css.NumberTerm) {
        radiusValueList[index] = double.tryParse(element.text) ?? 0.0;
      } else if (element is css.PercentageTerm) {
        radiusValueList[index] = double.tryParse(element.text) ?? 0.0;
      } else if (element is css.EmTerm) {
        radiusValueList[index] = double.tryParse(element.text) ?? 0.0;
      } else if (element is css.LiteralTerm) {
        radiusValueList[index] = double.tryParse(element.text) ?? 0.0;
      } else {
        radiusValueList[index] = 0.0;
      }
    });

    return BorderRadius.only(
        topLeft: Radius.circular(radiusValueList[0]),
        topRight: Radius.circular(radiusValueList[1]),
        bottomLeft: Radius.circular(radiusValueList[2]),
        bottomRight: Radius.circular(radiusValueList[3]));
  }

  /// todo. return null 값 확인필요
  // static Object? expressionToJustifyContents(String text) {
  //   switch (text) {
  //     case 'center':
  //     case 'end':
  //     case 'flex-start':
  ...
  //     case 'right':
  //     case 'start':
  //       return expressionToAlignment(text);
  //     case 'space-around':
  //     case 'space-between':
  //     case 'space-evenly':
  //       return expressionToMainAxisAlignment(text);
  //     default:
  //       return null;
  //   }
  //
  ...
  // static Alignment? expressionToJustifyContents(css.Expression text) {
  //
  //   print( text);
  //   // static Alignment? expressionToAlignment(String text) {
  //   switch (text) {
  //     case 'center':
  ...
  // static MainAxisAlignment? expressionToMainAxisAlignment(String text) {
  //   switch (text) {
  //     case 'space-around':
